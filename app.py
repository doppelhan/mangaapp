...
def adjust_gamma(image, gamma=1.0):
    try:
        ...
        logger.error(f"Error in adjust_gamma: {e}")
        raise e
...
def generate_mask(gray_image, pattern):
    try:
        ...
        logger.error(f"Error in generate_mask: {e}")
        raise e
...
def apply_screen_tone(image, size=5, pattern="None", mask=None, gray_image=None, color=(50, 50, 50), density=50, pencil_style="Light"):
    try:
            ...
            for y in range(0, height, spacing):
                for x in range(0, width, spacing):
                    if mask_array[y, x]:
                        ...
                        dx = length * np.cos(np.pi / 6)
                        dy = length * np.sin(np.pi / 6)
                        x_end = x + dx
                        y_end = y + dy
        ...
        logger.error(f"Error in apply_screen_tone: {e}")
        raise e
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            try:
                ...
                # สร้างและบันทึกภาพ preview (ขนาดจริง)
...
# Route สำหรับประมวลผลพารามิเตอร์เพิ่มเติม
...
# Route สำหรับบันทึกและดาวน์โหลดภาพที่ประมวลผลแล้ว
...
# Error handler สำหรับ RequestEntityTooLarge (413)
...
# Error handler สำหรับข้อผิดพลาดทั่วไป
